...
S(c(1, 2, 3, 5), mc = 10000)          # survival at multiple t values
```

A different `mc` triggers a fresh draw:

```{r}
S(2, mc = 10000)                       # uses the same cache
S(2, mc = 20000)                       # new cache with 20000 samples
```
...
set.seed(1)
mc_est <- algebraic.dist::surv(sys)(3, mc = 50000)
# Exact via Gamma
